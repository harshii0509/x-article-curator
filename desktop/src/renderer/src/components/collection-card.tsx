...
export function CollectionCard({ collection, className, ...props }: CollectionCardProps) {
  const linkLabel = collection.isPublic ? "Public" : "Private";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <h2 className="truncate text-sm font-semibold text-ns-ink">{collection.title}</h2>
      ...
      {collection.description ? (
        <p className="line-clamp-2 text-xs text-ns-ink/60">{collection.description}</p>
      ) : null}
    </article>
  );
}
